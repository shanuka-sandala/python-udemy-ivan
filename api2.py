...
url = "https://opentdb.com/api.php?amount=1&category=18&difficulty=easy"
endGame = ""

while endGame != "quit":
	r = requests.get(url)
	if r.status_code != 200:
		endGame = input("Sorry, there was a problem retrieving the data")
	else:
		valid_answer = False
		answer_number = 1
		...
		if user_answer == correct_answer:
			...
			print("Sorry, " + html.unescape(user_answer) + " is incorect, the correct answer is " + html.unescape(correct_answer))
			score_incorrect += 1
		...
		endGame = input("\nPress enter to try again or type 'quit' to quit the game. ").lower()
